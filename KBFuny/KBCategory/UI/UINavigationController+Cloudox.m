...
- (void)setNeedsNavigationBackground:(CGFloat)alpha {
    ...
    NSLog(@"******* setNeedsNavigationBackground START ********");
...
//                [NSStringFromClass([view class]) containsString:@"UINavigationBarModernPromptView"]||
//                [NSStringFromClass([view class]) containsString:@"UINavigationBarLargeTitleView"]) {
//                view.alpha = 0;
//            }
            view.alpha = alpha;
        }
     ...
     NSLog(@"******** setNeedsNavigationBackground END****************");
    ...
    if (self.navigationBar.isTranslucent) {
        if (backgroundImageView != nil && [backgroundImageView isKindOfClass:[UIImageView class]] && backgroundImageView.image != nil) {
            ...
            NSInteger index = [barBackgroundView subviews].count > 1 ? 1 :0;
            UIView *backgroundEffectView = [[barBackgroundView subviews] objectAtIndex:index];// UIVisualEffectView
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
            if ( KB_SYS_VERSION_GREATER_THAN_OR_EQUAL_TO(@"10.0.0")) {
                //otifyWhenInteractionChangesUsingBlock是10.0以后的api，换成notifyWhenInteractionEndsUsingBlock
