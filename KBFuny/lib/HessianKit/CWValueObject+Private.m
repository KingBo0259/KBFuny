...
static BOOL CWAddProtocolImplementationsToClass(Class aClass, Protocol* aProtocol) {
  ...
  for (int index = 0; success && index < count; index++) {
    ...
    }else if (type==@encode(uint32_t)[0]){
    
        getterSEL=@selector(priv_uint32Value);
        setterSEL=@selector(priv_setUint32Value:);
        NSLog(@"uint32");
    }else if (type==@encode(uint64_t)[0]){
        getterSEL=@selector(priv_uint64Value);
        setterSEL=@selector(priv_setUint64Value:);
        NSLog(@"uint64");
    }else if(type==@encode(Class)[0]){
    
        NSLog(@"Class:%@",propertyName);
    }
...
-(void)priv_setUint32Value:(uint32_t)value{
    [_instanceVariables setObject:[NSNumber numberWithInt:value] forKey:CWPropertyNameFromSelector(_cmd)];
}
...
-(void)priv_setUint64Value:(uint64_t)value{
    [_instanceVariables setObject:[NSNumber numberWithLongLong:value] forKey:CWPropertyNameFromSelector(_cmd)];
}
